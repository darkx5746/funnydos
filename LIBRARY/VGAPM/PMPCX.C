...
int vgaPmPcxCutDisp( char *fname, int skipColor,
				int putX1, int putY1, int putX2, int putY2 )
{
	...
	for ( i = putY1; i < putY1 + p->y2 - p->y1 + 1; i++ )
	{
        ...
        for( j = 0; j < n; j++ )
		{
			...
			vgaPmPutPixel( putX1 + j, i, *( temp + j ) );
		}
	}
...
int vgaPmPcxZoomDisp( char *fname, int skipColor,
				int putX1, int putY1, int putX2, int putY2 )
{
	FILE    *fp;
	PCXHDR  *p;
    ...
    for ( i = y; i < y + p->y2 - p->y1; i++ )
	{
			...
			( p->y2 - p->y1 ) < ( putY2 - putY1 ) )
		{
            ...
            for( j = 0; j < n; j++ )
                vgaPmPutPixel( putX1 + j, count+y, *( temp + j ) );
